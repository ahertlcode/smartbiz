...
*/ 
    ...
    public function view($critcol=null, $critval=null) {
        $db = new DbHandlers();
        if(is_null($critcol)){
            $sql = "SELECT * from sales_ledger order by id DESC";
        } else {
        $sql = "SELECT * from sales_ledger WHERE $critcol ='{$critval}'";
        }
    ...
    public  function delete($critcol, $critval){
        $db = new DbHandlers();
        $sql = "DELETE FROM sales_ledger WHERE $critcol ='{$critval}'";
